...
if (!isset($_SESSION['username'])) {
  header("location:home.php");
    ...
    if ($sex == 'B')
    {
      $sql1 = "SELECT *FROM `user_details` WHERE `city`= '$city' AND `sex` IN ('M','F') AND `uname`<> '$username' ORDER BY `fname`";
      $result1 = mysqli_query($conn, $sql1);
      $count = mysqli_num_rows($result1);
    }
    else
    {
      $sql1="SELECT *FROM `user_details` WHERE `city`= '$city' AND `sex`='$sex' AND `uname`<> '$username' ORDER BY `fname`";
      $result1=mysqli_query($conn,$sql1);
      $count=mysqli_num_rows($result1);
    }
    if ($count==0)
...
<html lang="en">
  ...
  <body>
    ...
    <div class="heading">Matches based on City</div>
