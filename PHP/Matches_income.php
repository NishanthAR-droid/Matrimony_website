...
if (!isset($_SESSION['username'])) {
  header("location:home.php");
    ...
    $min_income=$row['min_annual_income'];
    $max_income=$row['max_annual_income'];
    $sex=$row['sex'];
    $i=1;

    if ($sex == 'B')
    {
      $sql1 = "SELECT *FROM `user_details` WHERE `annual_income` BETWEEN $min_income AND $max_income AND `sex` IN ('M','F') AND `uname`<> '$username' ORDER BY `fname`";
      $result1 = mysqli_query($conn, $sql1);
      $count = mysqli_num_rows($result1);
    }
    else
    {
      $sql1="SELECT *FROM `user_details` WHERE `annual_income` BETWEEN $min_income AND $max_income AND `sex`='$sex' AND `uname`<> '$username' ORDER BY `fname`";
      $result1=mysqli_query($conn,$sql1);
      $count=mysqli_num_rows($result1);
    }
    if ($count==0)
...
<html lang="en">
  ...
  <body>
    ...
    <div class="heading">Matches based on Income</div>
    <div class="table_responsive">
      <table>
        ...
        <tbody>
            ...
            <td><?php echo $row1['annual_income'] ?></td>
            <td>
