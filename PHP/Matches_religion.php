...
if (!isset($_SESSION['username'])) {
  ...
  if ($sex == 'B') {
    $sql1 = "SELECT *FROM `user_details` WHERE `religion`= '$religion' AND `sex` IN ('M','F') AND `uname`<> '$username' ORDER BY `fname`";
    $result1 = mysqli_query($conn, $sql1);
    $count = mysqli_num_rows($result1);
  } else {
    $sql1 = "SELECT *FROM `user_details` WHERE `religion`= '$religion' AND `sex`='$sex' AND `uname`<> '$username' ORDER BY `fname`";
    $result1 = mysqli_query($conn, $sql1);
    $count = mysqli_num_rows($result1);
  }
...
<body>
  ...
  <div class="heading">Matches based on Religion</div>
